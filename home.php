...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_FILES["file"])) {
        ...
        if (!in_array($fileMimeType, $allowedMimeTypes)) {
            $message =
                "Invalid file type. Allowed types: text, PDF, images, and Word documents.";
        } else {
            // Sanitize file name
        ...
        $selectedFile = realpath($uploadDir . "/" . basename($_POST["file"]));
        if (
            $selectedFile === false ||
            strpos($selectedFile, $uploadDir) !== 0
        ) {
            die("Invalid file path.");
        }
            ...
            if (empty($password)) {
                ...
                if ($_POST["operation"] === "encrypt") {
                    ...
                    if ($encrypted === false) {
                        ...
                        $outputPath =
                            realpath($uploadDir) . "/" . basename($outputName);
                        if (
                            $outputPath === false ||
                            strpos($outputPath, $uploadDir) !== 0
                        ) {
                            die("Invalid output file path.");
                        }
                        $outputData = base64_encode(
                            $iv . "::" . $_POST["file"] . "::" . $encrypted
                        );
                    ...
                    if (!$decoded || strpos($decoded, "::") === false) {
                        ...
                        [$iv, $storedName, $encryptedData] = explode(
                            "::",
                            $decoded,
                            3
                        );
                        ...
                        if ($decrypted === false) {
                            ...
                            $outputName =
                                $storedName ?: $_POST["file"] . ".dec"; // Restore or default to .dec
                            $outputPath =
                                realpath($uploadDir) .
                                "/" .
                                basename($outputName);
